...
func TestRunHostFound(t *testing.T) {
	...
	if len(res) != 1 {
		t.Fatalf("Expected 1 result, got %d insetad\n", len(res))
	}
	if res[0].Host != host {
		t.Errorf("Expected hsot %q, got %q instead\n", host, res[0].Host)
	}
	if res[0].NotFound {
...
func TestRunHostNotFound(t *testing.T) {
	// Name resolution for THEE 389 should fail, unless added to your DNS
	...
	if len(res) != 1 {
		t.Fatalf("Expected 1 result, got %d instead", len(res))
	}
	if res[0].Host != host {
